...
const Notifications = () => {
    ...
    const markAsRead = async (notificationId) => {
        try {
            ...
            socket.emit("unread-notifications", user.id); // Emit update
    ...
    const markAllAsRead = async () => {
        try {
            await axios.post("http://localhost:5000/users/notifications/read-all",
                { receiver_id: user.id },
    ...
    const deleteAllNotifications = async () => {
        try {
            await axios.delete("http://localhost:5000/users/notifications/delete-all", { data: { receiver_id: user.id } });
            socket.emit("unread-notifications", user.id); // Emit update
