...
from backend.models.user import get_tickets_stats
...
def serialize_ticket(ticket):
    """Convert datetime objects to UTC before serialization."""
    return {
...
def emit_ticket_assigned(user_id):
    ...
    # print("Type of first ticket:", type(tickets[0]) if tickets else "Empty List")

    serialized_tickets = [serialize_ticket(ticket) for ticket in tickets["active_tickets_list"]]

    print(tickets)
    emit("inprogress-update", {"user_id": user_id, "metrics": {  # Send all ticket stats under 'metrics'
        ...
        },"tickets":serialized_tickets}, broadcast=True)
